...
import clientapi.subscriptions.TestSubscription
...
val graphQlDi = DI.Module("graphql") {
    ...
    bind<TestSubscription>() with singleton { TestSubscription() }
    ...
    bind<GraphQLSchema>() with singleton {
        ...
        toSchema(config = config, queries = queries, mutations = emptyList(), subscriptions = subscriptions)
    }
    ...
    bind<GraphQL>() with singleton {
        newGraphQL(instance()).defaultDataFetcherExceptionHandler(instance())
